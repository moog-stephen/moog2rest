        ...
        highlightMatches: function () {
            ...
            return function () {
                ...
                if (
                    ...
                    (!search.length && !o.invertMatches.checked) || !o.highlightMatches.checked
                ) {
                    f.input.clearBg();
                    return;
                }
                ...
                /* Update link for CSV export */
                var save = output;
                csv = save
                    .replace(re.matchPair, "$1%0A$3%0A");
                document.getElementById('export_link').href = "data:application/csv;charset=utf-8," + csv;
...
var parseRegex = function () {
    /* Even for JavaScript's relatively simple flavor, regex syntax parsing is complicated
     ...
     context-sensitivity issues. This code is tailored for RegexReplace's needs (e.g., no
        ...
        while (match = parser.exec(parts.contents)) {
            ...
            if (m.charAt(0) === "\\") {
                ...
                if (XRegExp.cache("^\\\\[cux]$").test(m)) {
                    ...
                    output += "<b>" + m + "</b>";
                    /* Traditional regex behavior is that a metaclass should be unrangeable (RegexReplace terminology).
                ...
                if (lastToken.rangeable) {
                    ...
                    if (nextToken) {
                        ...
                        if (parts.closing) {
                            output += "-"; // Since this is a literal, it's technically "rangeable," but that doesn't
                                           // matter
    ...
    return function (value) {
        ...
        while (match = re.regexToken.exec(value)) {
            ...
            switch (m.charAt(0)) {
                ...
                // Group opening
                case "(":
                    // If this is an invalid group type, mark the error and don't count it towards group depth or total
                    // count
                    if (m.length === 2) { // m is "(?"
                        output += errorStr(m);
                    }
                ...
                case "\\":
                    ...
                    if (XRegExp.cache("^[1-9]").test(m.charAt(1))) {
                        /* What does "\10" mean?
                         ...
                         and RegexReplace does not implement its own regex engine, it needs to highlight the regex as the
...
function replace_function() {
    var a = document.getElementById("replace_field").value;
    console.log(RegexReplace.fields.search.textbox.value);
    var search_ = RegexReplace.fields.search.textbox.value;
    var replace_value_ = document.getElementById("replace_field").value;
    ...
    try {
        ...
        var searchRegex = new XRegExp(re.sansTrailingAlternator.exec(search_)[0],
            ""
        );


        RegexReplace.fields.input.textbox.value = RegexReplace.fields.input.textbox.value.split(searchRegex).join(replace_value_);
        RegexReplace.highlightMatches();
...
/* SmartField is my name for the rich text boxes RegexReplace creates, which
...
function SmartField(el) {
    ...
    /* The <textarea> element already exists for graceful-degradation reasons.
     Not that RegexReplace would work at all without JavaScript, but whatever. */
    ...
    if (textboxEl.spellcheck) {
        textboxEl.spellcheck = false;
    }
...
extend(SmartField.prototype, {
    setBgHtml: function (html) {
        // Workaround an IE text-normaliztion bug where a leading newline is removed (causing highlighting to be
        // misaligned)
        if (isIE) {
    ...
    _onKeyUp: function (e) {
        ...
        if (this._matchOnKeyUp) {
            ...
            switch (srcEl) {
                case RegexReplace.fields.search.textbox: // fallthru
                case RegexReplace.fields.input.textbox:
                    RegexReplace.highlightMatches();
                    break;
    ...
    _testKeyHold: function (e) {
        ...
        if (this._keydownCount > 2) {
            RegexReplace.fields.input.clearBg();
            ...
            switch (srcEl) {
                ...
                case RegexReplace.fields.input.textbox:
                    setTimeout(function () {
                        RegexReplace.highlightMatches.call(RegexReplace);
                    }, 0);
                    break;
                // There might be other elements to handle in the future
            }
...
(function () {
    ...
    RegexReplace.highlightSearchSyntax();
    RegexReplace.highlightMatches();

    for (var flag in o.flags) {
        o.flags[flag].onclick = RegexReplace.highlightMatches;
    }

    o.highlightSyntax.onclick = RegexReplace.highlightSearchSyntax;
    o.highlightMatches.onclick = RegexReplace.highlightMatches;
    o.invertMatches.onclick = RegexReplace.highlightMatches;
    ...
    refBox.onmouseover = function (e) {
        refDropdown.onmouseover();
    };
    refBox.onmouseout = function (e) {
        refDropdown.onmouseout();
    };
    ...
    var replaceDropdown = $("replaceDropdown"),
        replaceBox = $("replace_box");

    replaceDropdown.onmouseover = function (e) {
        removeClass("hidden", replaceBox);
        addClass("hover", this);
    };
    replaceDropdown.onmouseout = function (e) {
        if (!hasClass("pinned", replaceBox)) {
            addClass("hidden", replaceBox);
            removeClass("hover", this);
        }
    };

    replaceBox.onmouseover = function (e) {
        replaceDropdown.onmouseover();
    };
    replaceBox.onmouseout = function (e) {
        replaceDropdown.onmouseout();
    };
    ...
    if (isIE6) {
        ...
        optionsDropdown.onmouseenter = function () {
            addClass("hover", this);
        };
        optionsDropdown.onmouseleave = function () {
            removeClass("hover", this);
        };
